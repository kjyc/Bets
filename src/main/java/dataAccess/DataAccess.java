...
public class DataAccess {
	...
	/**
	 * Funciï¿½n que busca un evento por su descripcion y fecha
	 * 
	...
	public void updateSport(Sport sp, Event ev) {
		...
		db.persist(sp1);
		db.getTransaction().commit();
		System.out.println("Evento aï¿½adido a deporte");
	}
	...
	public void updateUserBalance(User userLogeado, float bet) {
		...
		db.persist(u);
		db.getTransaction().commit();
		System.out.println("Usuario con saldo actualizado en DB: " + u.getUsBalance() + "ï¿½ï¿½");
	}
